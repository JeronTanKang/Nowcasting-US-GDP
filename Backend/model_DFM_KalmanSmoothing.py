...
target_variable = "GDP"
df_gdp = df.set_index("date")[[target_variable]]  # Keep GDP separately (including NaNs)

# Select all economic indicators except GDP
...
def apply_kalman_smoothing(series):
    ...
    """
    if series.isna().all():  # If the entire series is NaN, return as is
        return series
    ...
    # Ensure that all NaNs in the original series are filled with the smoothed values
    series_interpolated = series.combine_first(smoothed_values)

    return series_interpolated


# ---- Ensure Indicators Are Stationary Before DFM ---- #
def make_stationary(df):
    ...
    for col in df.columns:
        # ✅ Step 1: Apply Kalman smoothing first
        stationary_df[col] = apply_kalman_smoothing(stationary_df[col])

        # ✅ Step 2: Perform ADF test for stationarity
        adf_test = adfuller(stationary_df[col].dropna(), autolag="AIC")
        p_value = adf_test[1]

        # ✅ Step 3: If still non-stationary, apply differencing
        if p_value > 0.05:
            print(f"⚠️ {col} is still non-stationary after Kalman smoothing (p={p_value:.3f}). Applying differencing.")
            print(stationary_df[col].tail())
            stationary_df[col] = stationary_df[col].diff()

            # ✅ Step 4: Apply Kalman smoothing again to correct NaNs caused by differencing
            stationary_df[col] = apply_kalman_smoothing(stationary_df[col])

    # Drop rows where more than 60% of columns are NaN
    threshold = int(0.4 * df.shape[1])
    stationary_df = stationary_df.dropna(thresh=threshold)

    # Fill remaining NaNs using forward & backward fill (limited to 1 step)
...
# Drop initial rows that still contain too many NaNs
df_filtered = df_filtered.iloc[5:].dropna()

# Ensure at least 2 indicators remain
if df_filtered.shape[1] < 2:
    print("WARNING: Restoring non-stationary indicators to ensure DFM has at least 2 columns.")
    df_filtered = df.set_index("date")[indicators]  # Use original indicators if needed
